...
| Corpus | `corpus/` — 6 simplified news items (32 clauses), each validated through the parser; `corpus-check` CI gate enforces grammar + real-vocab-only (now incl. compounds); batch 2 exercises `okoa`/`pelesiden`/`menteli`, `lahili`/`tokea`, the `dobukebunka` zoo compound, `NUM + -pe` ordinals, `mungi`+numeral, and dates; gaps logged in `corpus/GAPS.md` |
