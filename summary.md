...
| Both gates | green (linter R1–R6 + collision checker); roots 1477/1477 + derived 10757/10757 + compounds 33/33 clear, exit 0 |
...
| Corpus | `corpus/` — 12 items (99 clauses), each parser-validated; `corpus-check` CI gate enforces grammar + real-vocab-only (incl. compounds + native-badge-ending roots). News batches + longer discourse pieces + an **opinion** op-ed (0009) + grammar-exercise pieces + a large 27-clause disaster report (0012, Delhi fire). `corpus/GAPS.md` logs the edges: comparative-with-standard, relative clauses, modality, embedded-"that"; vocab (foreign/trapped/firefighter/capital/investigate/spread/survive); and a role-marked-NP + post-nominal-`-pe` parser limit |
