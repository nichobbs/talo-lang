...
| Concepts catalogued | 1501 (`data/concepts.tsv`) |
| Lexicon forms minted | 1484 roots (`data/lexicon.tsv`) — **every catalogued root covered, 0 gaps** |
| Derived dictionary | 10811 affix-derived (`data/derived-lexicon.tsv`) + 33 curated compounds (`data/compounds.tsv`), all generated — **12328 total entries** |
| Both gates | green (linter R1–R6 + collision checker); roots 1484/1484 + derived 10811/10811 + compounds 33/33 clear, exit 0 |
| Conflict/diplomacy batches | +7 roots across two batches — b1: `katasa` forbid, `hamala` attack, `buki` weapon; b2: `huduna` ceasefire (Semitic), `milisi` militia (Austronesian), `misile` rocket (Romance), `kisasi` revenge (Bantu). Much of the domain (`setuyu` agree, `salama` safe, `guntai` army, `adui` enemy, `pelan` war, `amani` peace, `kosoa` negotiate) already existed. Both gates exit 0 |
