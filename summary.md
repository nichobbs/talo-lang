...
| Concepts catalogued | 1491 (`data/concepts.tsv`) |
| Lexicon forms minted | 1474 roots (`data/lexicon.tsv`) — **every catalogued root covered, 0 gaps** |
| Derived dictionary | 10727 affix-derived (`data/derived-lexicon.tsv`) + 33 curated compounds (`data/compounds.tsv`), all generated — **12234 total entries** |
| Both gates | green (linter R1–R6 + collision checker); roots 1474/1474 + derived 10727/10727 + compounds 33/33 clear, exit 0 |
...
| Derivation expansion | `docs/0007` — 3 layers, all gated/regenerable: (A) first-order affixes (incl. the `num` ordinal `-pe`) + (B) curated 2nd-order stacks via `scripts/derive-lexicon.mjs` → 10727 forms; (C) curated compounds via `scripts/derive-compounds.mjs` → 33. 1474 roots → 12234 entries. Wired into the dictionary build (`dictionary/`) so lookup resolves derived/compound words |
| Corpus | `corpus/` — 8 simplified news items (53 clauses), each validated through the parser; `corpus-check` CI gate enforces grammar + real-vocab-only (incl. compounds). Batch 2 exercises the new vocab/ordinals; batch 3 adds two **longer** pieces stressing discourse grammar — conditionals (`fi…toki`), wh- and yes/no questions, connectives (`i`/`ma`/`sababu`), negative imperative; gaps logged in `corpus/GAPS.md` |
