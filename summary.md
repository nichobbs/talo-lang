...
| Concepts catalogued | 1507 (`data/concepts.tsv`) |
| Lexicon forms minted | 1486 roots (`data/lexicon.tsv`) — **every catalogued root covered, 0 gaps** |
| Derived dictionary | 10826 affix-derived (`data/derived-lexicon.tsv`) + 34 curated compounds (`data/compounds.tsv`), all generated — **12346 total entries** |
| Both gates | green (linter R1–R6 + collision checker); roots 1486/1486 + derived 10826/10826 + compounds 34/34 clear, exit 0 |
...
| Derivation expansion | `docs/0007` — 3 layers, all gated/regenerable: (A) first-order affixes (incl. the `num` ordinal `-pe`) + (B) curated 2nd-order stacks via `scripts/derive-lexicon.mjs` → 10826 forms; (C) curated compounds via `scripts/derive-compounds.mjs` → 34. 1486 roots → 12346 entries. Wired into the dictionary build (`dictionary/`) so lookup resolves derived/compound words |
