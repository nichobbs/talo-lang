...
### 5.1 Derived numeral series — ordinal, multiplicative, fraction (`docs/0010`)

Three further series are built on the bare cardinals, with **no new numeral
words** (rule 5). All four ratified by `docs/0010` (the multiplicative/fraction
markers via the `0011` Tier-S governance path):

| Series | Construction | Example | Gloss |
|---|---|---|---|
| **cardinal** (counting) | bare numeral, postposed (`0005` §3) | `liboka mo` | three books |
| **ordinal** | `NUM + -pe` (modifier badge) | `liboka mope` | the third book |
| **multiplicative** | `NUM kai` (post-verbal marker) | `mi hasiluto mo kai` | I ran three times |
| **fraction** | `NUM bagi NUM` (divide marker) | `panika ta bagi mo` | a third of the water |

- The **ordinal** is the modifier reading of a cardinal — the `-pe` badge on a
  numeral root — and is generated by the `0007` `num` paradigm; no marker word.
- **`kai`** (Japanese 回 "times") is an invariant closed-class marker following the
  numeral, post-verbal like an aspect particle (`0002` §5.1).
- **`bagi`** (Indonesian "divide/share") is the divide relation: `a bagi b` = a÷b;
  the only memorised fraction is the ultra-frequent `setenga` "half" (`QTY-027`).
- Both markers are **closed-class grammatical words**: reserved in the collision
  checker and recognised by the parser (`numeralMarker`), **not** lexicon dataset
  rows — consistent with how 0002 Appendix-B morphemes are handled (`0010` §6).
